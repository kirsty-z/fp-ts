...
import './styles.css' // mostly with tailwind and a little vanilla css
...
const useFocusAtom = (anAtom: PrimitiveAtom<Food>) => {
  /*
    ...
    that's why we use `useUpdateAtom`, the `get` value can obtain
    by `foodAtom` below
  */
    ...
    >
      ...
      <div className="px-4 py-1 md:py-2">
        <div className="bg-amber-400 rounded">
          <span className="inline-block mt-2 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">amount</span>
          <input
            type="number"
...
const ItemList = ({ foodListAtom }: { foodListAtom: PrimitiveAtom<Food[]> }) => {
  /*
        ...
        <ul className="my-5 flex">
          {foodList.map((food: Food) => {
            return (
              ...
              >
                <strong>{food.name}</strong> : ${(parseInt(food.amount, 10) * food.price).toFixed(2)}
              </p>
            )
          })}
        </ul>
